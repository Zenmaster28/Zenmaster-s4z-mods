...
export class SauceElevationProfile {
    ...
    setRoute = common.asyncSerialize(async function(id, {laps=1, eventSubgroupId}={}) {
        ...
        if (typeof(eventSubgroupId) == "undefined")
        {
            eventSubgroupId = 0;
        }
        this._roadSigs = new Set();
        ...
        let distances = Array.from(this.route.distances);
        let elevations = Array.from(this.route.elevations);
        let grades = Array.from(this.route.grades);     
        ...
        {
            let sgInfo = await common.rpc.getEventSubgroup(eventSubgroupId);
            //debugger
            sgInfo.distanceInMeters ? this.customDistance = sgInfo.distanceInMeters : "";
            //this.customDistance = 95000
            console.log(this.customDistance, sgInfo.routeDistance)
            if (this.customDistance > 0 && this.customDistance < distances[distances.length - 1])
            {
                //custom distance is less than the full route                
                let cdIdx = distances.findIndex(x => x > this.customDistance)                
                if (cdIdx + 5 <= distances.length - 1)
                {
                    cdIdx = cdIdx + 5; // show a little more than the expected distance
                }
                else
                {
                    cdIdx = distances.length - 1;
                }
                console.log("Custom Distance: " + this.customDistance + " at index " + cdIdx)
                distances = distances.slice(0,cdIdx)
                elevations = elevations.slice(0,cdIdx)
                grades = grades.slice(0,cdIdx)
                this.curvePath.nodes = this.curvePath.nodes.slice(0,cdIdx)
                
            }
            else if (this.customDistance > 0 && this.customDistance > sgInfo.routeDistance)
            {
                //custom distance is greater than the full route
                let extraDistance = this.customDistance - distances[distances.length - 1];
                let lapDistance = distances.at(-1) - distances.at(lapStartIdx)
                let leadinDistance = distances.at(-1) - lapDistance;
                let extraLaps = parseInt(extraDistance / lapDistance)
                console.log("Extra laps: " + extraLaps)
                let cdIdx;
                //debugger
                for (let j = 0; j < extraLaps; j++)
                {       
                    laps++;   
                    cdIdx = distances.length - 1;
                    //let initMaxDistance = distances.at(-1) - distances.at(lapStartIdx)
                    for (let i = lapStartIdx; i < cdIdx; i++) {
                        distances.push(distances.at(-1) + (this.route.distances[i] - (this.route.distances[i - 1] || 0)));  
                        elevations.push(this.route.elevations[i]);
                        grades.push(this.route.grades[i]);                            
                    }
                    this.curvePath.extend(this.route.curvePath.slice(lapStartIdx, cdIdx)); // lets do something with this curvepath to calc segments via roadSegments?
                    //foreach roadsegment that is not leadin, get segments with marklines and extend curvepath, distances etc.
                    
                    extraDistance = extraDistance - lapDistance;                    
                    //debugger
                }
                //debugger
                if (extraDistance < lapDistance)
                {
                    cdIdx = distances.findIndex(x => x > extraDistance + leadinDistance);
                    cdIdx = cdIdx + 5;
                    for (let i = lapStartIdx; i < cdIdx; i++) {
                        distances.push(distances.at(-1) + (this.route.distances[i] - (this.route.distances[i - 1] || 0)));  
                        elevations.push(this.route.elevations[i]);
                    ...
                    this.curvePath.extend(this.route.curvePath.slice(lapStartIdx, cdIdx));
                    //debugger
                }
                //debugger
                for (let lap = 1; lap < laps + 1; lap++) {
                    if (this.showSegmentStart)
                    {
                        ...
                        {                    
                            ...
                            allMarkLines.push({name: segment.name, markLine: markLineIndex, id: segment.id})                
                            markLines.push({
                                xAxis: markLineIndex,
                                lineStyle: {width: this.lineSize, type: this.lineType},
                                label: {
                    ...
                    //}  
                        ...
                        {                     
                            ...
                            allMarkLines.push({name: segment.name + " Finish", markLine: markLineIndex, id: segment.id}) 
            ...
            if (this.routeOffset)
            {
                ...
                console.log("offset index: " + offsetIdx)
                //debugger
                markLines.push({
                    xAxis: distances[offsetIdx],
                    lineStyle: {width: this.lineSize, type: this.lineTypeFinish},
                    label: {
                        distance: 7,
                        position: 'insideEndTop',
        ...
        if (!watching && (this.courseId == null || (!this.road && !this.route))) {
            ...
            if (this.preferRoute) {
                if (watching.routeId) {
                    if (this.routeId !== watching.routeId ||
                        ...
                        if (sg && sg.routeId === watching.routeId) {   
                            ...
                            console.log("Setting route no sg")                          
                ...
                if (this.customDistance > 0 && this.deltas.length > 20 && !this.routeOffset && this.routeId != null)
                {
                    ...
                    console.log("Distance delta is: " + this.deltas.reduce((a, b) => a + b, 0) / this.deltas.length)
